...
export default function SocialStrip() {
  ...
  return (
    <section ref={sectionRef} className="py-20 lg:py-28 bg-ivory overflow-hidden">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 mb-10 text-center">
        <p className="font-mono text-xs text-gold tracking-[0.3em] uppercase mb-3">Threads, Elsewhere</p>
        <h2 className="font-heading text-4xl sm:text-5xl lg:text-6xl text-burgundy mb-3">
          @snugandknot
        </h2>
        <p className="text-wine text-sm">The weave extends beyond this frame</p>
      </div>

      <div ref={stripRef} className="flex gap-3 sm:gap-4 px-4 sm:px-6 lg:px-8">
        {socialImages.map((img, i) => {
          const isOdd = i % 2 === 0 // 0-indexed: first, third, fifth are "odd" items visually
          const cardWidth = isOdd ? 'w-[240px] sm:w-[280px] lg:w-[300px]' : 'w-[180px] sm:w-[200px] lg:w-[220px]'
          const rotation = isOdd ? 'rotate-[1deg]' : 'rotate-[-1deg]'
          ...
          return (
            <a
              ...
              className={`social-card group shrink-0 ${cardWidth} aspect-square rounded-xl overflow-hidden bg-parchment relative ${rotation} transition-transform duration-700 hover:rotate-0 hover:scale-105`}
            >
              <Image
                src={img.src}
                alt={img.alt}
              ...
              />
              {/* Hover overlay */}
              <div className="absolute inset-0 bg-burgundy/0 group-hover:bg-burgundy/30 transition-all duration-500 flex items-center justify-center">
                <div className="opacity-0 group-hover:opacity-100 transition-all duration-500 group-hover:translate-y-0 translate-y-2">
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    className="fill-gold-pale"
                  >
